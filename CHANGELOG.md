...
- 新增双端主动断开：提词端可"断开当前控制端"（轮换新 session/token/二维码）或"停止远控"（关闭 Server）；控制端顶部栏新增断开入口。显式断开后不触发自动重连，旧恢复凭据与旧连接回调立即失效，断开不影响本机播放。
- 控制端页面重构：速度控制改为左右方形图标按钮 + 中央速度值（不再把 "−0.1×/+0.1×" 数字塞进按钮导致多余"0"）；微调/暂停/恢复/长按结束区域改为自适应布局（窄屏纵向排列），长按结束使用紧凑危险区；内容区可滚动并应用底部导航栏 Insets。
- 提词线附近文字改为真实定位：从正式播放页的 `TextLayoutResult` 按提词线在视口中的真实 Y（`guideLinePosition` + 当前滚动偏移）找到穿越的视觉行，取前/当前/后最多 3 行纯文本；随滚动、跳转、调速、字号、横竖屏、镜像实时更新；仅在锚点行或文本变化时发布，不再使用静态 `plainTextPreview` 摘要。
