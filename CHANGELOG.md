...
- 正式播放第一行初始位置改为按"开始播放那一刻"的提词线状态确定：提词线开启时第一行位于提词线下约 1.5 个真实行高处；关闭时位于正文视口顶部约四分之一处。播放开始后移动或开关提词线不再影响正文滚动几何、进度、已用/剩余时间或滚动距离。
...
- 控制端"提词线附近文字"不再包含提词端视觉折行：改为从原始文本按字符范围截取连续切片，只保留源文本中真实存在的段落/手动换行；控制端按自身宽度重新自然排版，不再出现单字孤行。
- 控制端附近文字区域固定高度：按真实 `lineHeight × 固定行数`（窄屏/大字体 4 行，其余 5 行）计算，进度条与时间行不再随文字长度上下跳动。
- 提词线附近文字改为真实定位：从正式播放页的 `TextLayoutResult` 按"开始播放时捕获的稳定阅读锚点"（非实时提词线位置）找到穿越的视觉行，取前/当前/后最多 3 行纯文本；随滚动、跳转、调速、字号、横竖屏、镜像实时更新；仅在锚点行或文本变化时发布，不再使用静态 `plainTextPreview` 摘要。
