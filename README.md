...
- 手机远控（一对一同局域网）：提词端作为 WebSocket Server 生成一次性配对二维码，控制端扫码建立连接；支持协议握手、心跳、断线自动重连（有限指数退避）、单控制端限制、状态快照同步、真实播放命令和”设置保存后”远控启动。双端支持主动断开（提词端可断开当前控制端或完全停止远控，控制端可断开提词端），显式断开不会触发自动重连；控制端页面针对窄屏和字体缩放做了响应式优化。正式播放第一行按”开始时提词线状态”定位（开启时位于提词线下约 1.5 行、关闭时位于视口顶部约四分之一处），播放开始后移动/开关提词线不影响正文滚动。控制端"当前朗读文本"采用**绝对阅读游标 + 持续滑动阅读窗口**同步：提词端基于真实排版与固定阅读锚点持续计算全文绝对 UTF-16 阅读游标（低频大阅读窗口 + 高频 latest-only 游标，约 12–20 Hz），阅读窗口按游标前后阈值（前进约 72% / 后退约 18%）持续向前/向后滑动、窗口 revision 严格递增，控制端用本地 `TextLayoutResult` 重新排版窗口文本并平滑滚动到固定阅读锚点，支持 pending cursor（游标先于覆盖窗口到达时暂存、覆盖窗口到达后重锚定），长台本可持续读到结尾、窗口切换不产生空白或回跳，不再保留提词端视觉折行、显示区域高度固定、不建立第二播放时钟。局域网传输当前不加密，请仅在可信 Wi-Fi 或个人热点使用。提词设置页底部的控制端状态卡片已接入远控入口，点击前会先保存当前播放设置。
