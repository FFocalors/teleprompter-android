...
项目处于早期开发阶段，尚未发布正式稳定版本，当前适合开发测试和功能体验。台本管理、编辑、播放设置、本地播放主流程和 TXT/DOC/DOCX 文件导入已经接入真实本地数据层；Markdown 导入、网络远控和语音跟随仍未完成。
...
- DOC/DOCX 文件导入：同一“导入文件”入口，格式由文件内容自动识别；DOCX 保留粗体/斜体/下划线、表格和超链接显示文字，DOC 优先保证正文与段落完整（格式有限降级）。
...
DOC 与 DOCX 导入遵循同样的隐私原则：仅读取用户主动选择的文件，解析完成后只保存转换后的台本正文与基础样式，原文件不复制、不持续关联。Word 文件解析全部在本地完成，不联网、不上传。
