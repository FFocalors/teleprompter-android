...
- 手机远控（一对一同局域网）：提词端作为 WebSocket Server 生成一次性配对二维码，控制端扫码建立连接；支持协议握手、心跳、断线自动重连（有限指数退避）、单控制端限制、状态快照同步、真实播放命令和”设置保存后”远控启动。双端支持主动断开（提词端可断开当前控制端或完全停止远控，控制端可断开提词端），显式断开不会触发自动重连；控制端页面针对窄屏和字体缩放做了响应式优化；控制端”附近文字”按提词端真实 `TextLayoutResult` 与提词线位置实时定位，不再使用静态摘要。局域网传输当前不加密，请仅在可信 Wi-Fi 或个人热点使用。
